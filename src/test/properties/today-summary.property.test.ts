    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (seed, deviceCount, recordDays) => {
          ...
          const devices: Device[] = expectOk(await provider.listDevices());
          expect(devices.length).toBeLessThanOrEqual(MAX_DEVICES);

          // 汇总（省略 deviceId）：当日全部设备求和
          const aggregate: DailySummary = expectOk(await provider.getTodaySummary());

          // 各设备分项：逐台调用 getTodaySummary(deviceId)
          let sumCharge = 0;
          let sumDischarge = 0;
          for (const device of devices) {
            const perDevice: DailySummary = expectOk(
              await provider.getTodaySummary(device.id)
            );
            // 分项值同样应为 2 位小数精度
