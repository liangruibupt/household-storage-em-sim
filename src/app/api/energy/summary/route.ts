// 充放电「当日总量」API 路由（任务 12.3，需求 3.1、3.4）
//
// Next.js App Router Route Handler：GET /api/energy/summary
// 支持可选查询参数 deviceId：
//   - 省略 deviceId  → 汇总名下全部设备当日总充/放电量
...
/**
 * GET /api/energy/summary[?deviceId=xxx]
 *
 * 返回当日（00:00 至当前）总充/放电量汇总。
 * deviceId 省略表示全部设备汇总（需求 3.1、3.4）。
 *
 * @returns 成功 200 { data: DailySummary }；未知设备 404、内部错误 500 { error }
 */
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    const result = await provider.getTodaySummary(deviceId);
